...
@push('styles')
<style>
    /* Styling Banner Seragam (Tema Outbound - Merah Soft / Coral) */
    .header-banner {
        border-radius: 10px;
        padding: 25px;
        color: white;
        ...
        position: relative; 
        overflow: hidden; 
        background: linear-gradient(135deg, #e57373, #ef5350); /* Warna merah pastel yang lembut di mata */
    }
    .header-banner-icon {
        position: absolute;
        right: -2%;
...
<div class="header-banner shadow-sm d-flex justify-content-between align-items-center">
    ...
    <div class="header-content">
        <h4 class="fw-bold mb-1"><i class="fa-solid fa-upload me-2"></i> Laporan Distribusi / Outbound</h4>
        <p class="mb-0 text-white-50 small">Pantau histori transaksi distribusi barang keluar secara spesifik per materiil.</p>
    </div>
    <div class="header-content">
        <a href="{{ route('reports.outbound.export', request()->all()) }}" class="btn btn-light fw-bold text-danger shadow-sm px-4 py-2" style="border-radius: 8px;">
    ...
    <table class="table-dense">
        <thead>
            <tr>
                ...
                <th class="text-center" width="20%">Total Terdistribusi (Keluar)</th>
                <th class="text-center" width="30%">Aksi / Histori SPPM</th>
            </tr>
        </thead>
        <tbody>
            @php $no = 1; @endphp
            @forelse($groupedOutbounds as $group)
                
                <!-- BARIS HEADER KATEGORI -->
                <tr>
                    <td colspan="4" class="category-header">
                        <i class="fa-solid fa-tags me-2 text-danger opacity-75"></i> {{ $group['category_name'] }}
                    </td>
                </tr>
                ...
                @foreach($group['items'] as $row)
                    <tr class="main-row">
                        ...
                        <td class="{{ $row['is_child'] ? 'child-material' : '' }}">
                            @if(!$row['has_children'] && $row['total_out'] > 0)
                                <button class="accordion-toggle collapsed fw-bold text-dark" type="button" data-bs-toggle="collapse" data-bs-target="#collapseMat{{ $row['material_id'] }}">
                                    <i class="fa-solid fa-chevron-right me-2 text-danger"></i>
                        ...
                        <td class="text-center align-middle">
                            ...
                            @else
                                <span class="badge bg-danger bg-opacity-10 text-danger border border-danger fw-bold px-3 py-1">
                                    <i class="fa-solid fa-arrow-up me-1"></i> {{ number_format($row['total_out'], 0, ',', '.') }} {{ $row['satuan'] }}
                                </span>
                            @endif
                        </td>
                        ...
                        <td class="text-center align-middle">
                            @if(!$row['has_children'] && $row['total_out'] > 0)
                                <button class="btn btn-sm btn-light border text-danger shadow-none accordion-toggle collapsed py-1 px-3 d-inline-block w-auto" type="button" data-bs-toggle="collapse" data-bs-target="#collapseMat{{ $row['material_id'] }}">
                                    <i class="fa-solid fa-list-check me-1"></i> Rincian ({{ count($row['transactions']) }} Data)
                                </button>
                            @elseif(!$row['has_children'])
                                <span class="text-muted small fst-italic">Belum ada distribusi</span>
                            @else
                                <span class="text-muted">-</span>
                            @endif
                        </td>
                    ...
                    <!-- ACCORDION: DETAIL HISTORI SPPM KELUAR -->
                    @if(!$row['has_children'] && $row['total_out'] > 0)
                    <tr class="border-0">
                        <td colspan="4" class="p-0 border-0">
                            <div class="collapse" id="collapseMat{{ $row['material_id'] }}">
                                <div class="nested-table-container px-4 py-3">
                                    <h6 class="fw-bold text-danger mb-2" style="font-size: 0.8rem;"><i class="fa-solid fa-truck-fast me-1"></i> HISTORI DISTRIBUSI BARANG KELUAR</h6>
                                    
                                    <div class="table-responsive bg-white border rounded">
                                        <table class="table nested-table mb-0 text-nowrap">
                                            <thead>
                                                <tr>
                                                    <th>Tgl Keluar Fisik</th>
                                                    <th>Nomor SPPM Keluar</th>
                                                    <th>Tujuan Pengiriman</th>
                                                    <th>Rentang Nomor Seri</th>
                                                    <th class="text-center text-danger border-start">Qty Keluar</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                        ...
                                                        <td class="align-middle">
                                                            @if($trx->seri_awal || $trx->seri_akhir)
                                                                <span class="d-inline-block text-muted" style="font-size: 0.7rem; background:#f8fafc; border: 1px solid #e2e8f0; border-radius:4px; padding:3px 8px;">
                                                                    {!! $formatSeri($trx->prefix, $trx->seri_awal, $trx->seri_akhir) !!}
                                                                </span>
                                                            @else
                                                                <span class="text-muted fst-italic small">Tidak memakai seri</span>
                                                            @endif
                                                        </td>
                                                        <td class="text-center fw-bold text-danger bg-danger bg-opacity-10 border-start align-middle">
                                                            -{{ number_format($trx->qty_keluar, 0, ',', '.') }}
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
            ...
            @empty
                <tr>
                    <td colspan="4" class="text-center py-5 text-muted bg-white">
                        ...
                        <span class="small fw-semibold">Belum ada data distribusi keluar untuk kriteria yang dipilih.</span>
                    </td>
                </tr>
            @endforelse
        </tbody>
